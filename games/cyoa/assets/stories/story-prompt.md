...
Choose one tone from the list below. Maintain it consistently across the story while allowing natural emotional variation within scenes (e.g., a suspenseful story can contain moments of relief or dark humor without changing its core tone).
...
The chosen tone should shape vocabulary, sentence rhythm, and narrative distance:
- A **suspenseful** story uses short declarative sentences, tight focus, and close narrative distance.
- A **whimsical** story uses playful diction, longer flowing sentences, and a lighter narrative presence.
- An **epic** story uses elevated vocabulary, wider scope, and measured cadence.
- A **melancholic** story uses longer, reflective sentences with muted imagery, dwelling on absence and memory rather than action.
- A **surreal** story uses unexpected juxtapositions, dream-logic transitions, and matter-of-fact delivery of impossible details.
- A **satirical** story uses precise, dry wit, ironic understatement, and a detached narrative voice that highlights absurdity through contrast.
- A **mysterious** story uses withholding and implication, measured pacing, and sensory details that suggest more than they reveal.

Match the voice to the genre. A sci-fi thriller should not read like a fairy tale.
...
2. Keywords section before chapters:
  ...
  - Exactly 3 bullet keywords:
    ...
    - `- keyword`
    - `- keyword`
  - Keywords should help a potential reader decide whether this story is worth reading. Choose words that convey the story's theme, setting, or tone — not generic terms. Each keyword must be a real word found in a standard dictionary.
...
3. Chapters:
  ...
  - Each chapter must contain all three subsections in this order:
    ...
    - `### Content`
    - `### Choices`
  - Chapter titles should hint at the chapter's content or mood without spoiling its key revelation

4. Choices format:
  - Non-ending chapter: numbered choices like `1. Do something -> 4`
  - Ending chapter: exactly `The End`
  - Each non-ending chapter must offer **2 choices**, occasionally **3** when the story demands it
...
5. Endings:
  - Total ending chapters using `The End`: **3 to 5**
  - At least 1 positive/success ending and 1 very bad ending

6. Branch depth:
  - Every path from Chapter 1 to an ending must pass through **at least 4 chapters** (including the ending chapter). No two-step shortcuts to an ending.
