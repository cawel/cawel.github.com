...
3. Run `01` and save output to `llm pipeline/output/01-concepts.json`.
4. Pick one concept from `01-concepts.json`.
5. Run `02` with chosen concept and save output to `llm pipeline/output/02-architecture.json`.
6. Run `03` with concept + architecture and save output to `llm pipeline/output/03-story-draft.md`.
7. Run `04` on draft and save output to `llm pipeline/output/04-critique.json`.
8. Run `05` with draft + critique and save output to `llm pipeline/output/05-story-final.md`.
9. Run `06` with final story and save output to `llm pipeline/output/06-metadata.json`.

## Output Files

Store each step result in `llm pipeline/output/` using these filenames:

| Step | File |
|------|------|
| `01` | `llm pipeline/output/01-concepts.json` |
| `02` | `llm pipeline/output/02-architecture.json` |
| `03` | `llm pipeline/output/03-story-draft.md` |
| `04` | `llm pipeline/output/04-critique.json` |
| `05` | `llm pipeline/output/05-story-final.md` |
| `06` | `llm pipeline/output/06-metadata.json` |
...
- After each step, write the output to its matching file in `llm pipeline/output/`.
