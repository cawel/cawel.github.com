...
- **Convergence stealth**: Convergence chapters must read as if the reader's current path is the only path. Every sentence commits to a single fact, a single emotional state, a single outcome — as though no other version of the story exists. This rule is violated not just by explicit branching language ("if you chose…", "depending on your earlier decision…") but equally by literary-sounding conditionals that expose the mechanic in disguise: "what she carries depends on what you gave her", "he says — or he says nothing", "the outcome traces back to your earlier choice". These forms are equally immersion-breaking because they remind the reader that other paths exist. A convergence chapter states what happened. Write that one thing.
